...
public class SportNewsListFragment extends Fragment {

    public final static String SHARED_PREF_NAME = "last_online_sports_news";

    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_news_list,container,false);
        SportsNewsApi sportsNewsApi = ApiClient.getClient().create(SportsNewsApi.class);
        SharedPreferences sharedPreferences = getContext().getSharedPreferences(SHARED_PREF_NAME, Context.MODE_PRIVATE);
        sportsNewsApi.getAllNews().enqueue(new Callback<List<News>>() {
            @Override
