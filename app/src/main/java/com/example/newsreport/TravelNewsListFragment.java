...
public class TravelNewsListFragment extends Fragment {

    public final static String SHARED_PREF_NAME = "last_online_travel_news";

    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_news_list,container,false);
        TravelNewsApi travelNewsApi = ApiClient.getClient().create(TravelNewsApi.class);
        SharedPreferences sharedPreferences = getContext().getSharedPreferences(SHARED_PREF_NAME, Context.MODE_PRIVATE);
        travelNewsApi.getAllNews().enqueue(new Callback<List<News>>() {
            @Override
